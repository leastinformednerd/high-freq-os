    ...
    */
    ...
    match Command::new("cargo")
        .args([
            ...
            "kern",
            "--config",
            "build.target = \"kern/os-dev-target.json\"",
            "--config",
        ...
        .code()
    {
        ...
        Some(status_code) => {
            println!("cargo build failed with status code {status_code}");
            return false;
        }
        None => {
            println!("cargo build did not terminate with status code (was terminated by signal)");
            return false;
        }
    }
        ...
        .code()
    {
        ...
        None | Some(_) => {
            println!("Failed to mount the disk image loopback");
            return false;
        }
    }
    ...
    match DEFERRED.lock() {
        ...
        Err(_) => {
            ...
            let _ = Command::new("umount").args(["./temporary_mount"]).spawn();
            return false;
        }
    }
    ...
    match Command::new("sudo")
        .args([
            "cp",
            "./target/os-dev-target/debug/kern",
            "./temporary_mount/kernel_entry",
        ])
        .status()
        ...
        .code()
    {
        ...
        None | Some(_) => {
            println!("Failed to copy the kernel entry over");
            return false;
        }
    }
        ...
        match *op {
            "losetup" => {
                let _ = Command::new("sudo")
                    ...
                    .output()
                    .expect("Failed to call deferred losetup -d");
            ...
            "mount" => {
                let _ = Command::new("sudo")
                    .args(["umount", "./temporary_mount"])
                    .output()
                    .expect("Failed to process deferred unmount");
                println!("Deferred unmount process");
            }
            other => {
                println!("Unexpectedly found '{other}' in the defer list");
                return false;
            }
        }
    }
...
fn test() {
    ...
    Command::new("qemu-system-x86_64")
        .args([
            ...
            "gtk,zoom-to-fit=on",
            "--bios",
            "/nix/store/y0c428fwc7z8bp5m36c3d00qcn3qyx8g-OVMF-202402-fd/FV/OVMF.fd",
            "-drive",
...
fn main() {
    match args()
        .nth(1)
        .expect("An argument must be provided to xtask")
        .as_str()
    {
        "build" => {
            deferred_build();
        ...
        cmd => {
            println!("Did not recognise command {cmd}");
        }
    };
}
